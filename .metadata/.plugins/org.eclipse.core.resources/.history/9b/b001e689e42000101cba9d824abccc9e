...
public class StockService {
	...
	public String removeAllStock(String email) {
		List<StockTradeDetail> byEmail = tradeDetailRepo.findByEmail(email);
		
		tradeDetailRepo.deleteAll(byEmail);
		return "Hi, "+byEmail.stream().map(s->s.getStockName()).toList()+" stocks removed from your portfolio and your portfolio is empty";
	}
}
